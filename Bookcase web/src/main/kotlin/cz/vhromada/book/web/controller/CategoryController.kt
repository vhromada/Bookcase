...
class CategoryController(
     ...
     */
    private val listRedirectUrl = "redirect:/categories/list"

    /**
     * Message for illegal request
     */
    private val illegalRequestMessage = "Category doesn't exist."
    ...
    fun showList(model: Model): String {
        ...
        model.addAttribute(titleAttribute, "Categories")

        return "category/index"
    }

    /**
     * Shows page with detail of category.
     *
     * @param model model
     * @param id    ID of editing category
     * @return view for page with detail of category
    ...
    fun showDetail(model: Model, @PathVariable("id") id: Int): String {
        ...
        if (category != null) {
            ...
            model.addAttribute(titleAttribute, "Category detail")
    ...
    fun showAdd(model: Model): String {
        return createFormView(model, CategoryFO(), "Add category", "add")
    }
    ...
    fun processAdd(model: Model, @ModelAttribute("category") @Valid category: CategoryFO, errors: Errors): String {
        Assert.isNull(category.id, "ID must be null.")

        if (errors.hasErrors()) {
            return createFormView(model, category, "Add category", "add")
        }
        processResults(categoryFacade.add(Category(category.id, category.name!!, category.position)))
    ...
    fun showEdit(model: Model, @PathVariable("id") id: Int): String {
        ...
        return if (category != null) {
            createFormView(model, CategoryFO(category.id, category.name, category.position), "Edit category", "edit")
        } else {
            throw IllegalRequestException(illegalRequestMessage)
        }
    }
    ...
    fun processEdit(model: Model, @ModelAttribute("category") @Valid category: CategoryFO, errors: Errors): String {
        Assert.notNull(category.id, "ID mustn't be null.")

        if (errors.hasErrors()) {
            return createFormView(model, category, "Edit category", "edit")
        }
        processResults(categoryFacade.update(processCategory(Category(category.id, category.name!!, category.position))))
