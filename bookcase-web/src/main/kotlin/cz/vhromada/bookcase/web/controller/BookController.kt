...
class BookController(
    ...
    fun processAdd(model: Model, @ModelAttribute("book") @Valid book: BookFO, errors: Errors): String {
        ...
        processResults(bookFacade.add(bookMapper.mapBack(book).copy(authors = getAuthors(book.authors!!), categories = getCategories(book.categories!!))))
    ...
    fun processEdit(model: Model, @ModelAttribute("book") @Valid book: BookFO, errors: Errors): String {
        ...
        processResults(bookFacade.update(processBook(bookMapper.mapBack(book).copy(authors = getAuthors(book.authors!!), categories = getCategories(book.categories!!)))))
     ...
     * Returns categories.
     *
     * @param source list of categories' ID
     * @return categories
     */
    private fun getCategories(source: List<Int?>): List<Category> {
        return source.map {
            val result = categoryFacade.get(it!!)
            processResults(result)
